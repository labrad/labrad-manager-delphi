object EnvironmentDialogForm: TEnvironmentDialogForm
  Left = 273
  Top = 166
  BorderStyle = bsToolWindow
  Caption = 'Server Name'
  ClientHeight = 169
  ClientWidth = 271
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 8
    Top = 48
    Width = 117
    Height = 13
    Caption = 'Please specify %<var>%:'
  end
  object Label5: TLabel
    Left = 24
    Top = 96
    Width = 217
    Height = 13
    Alignment = taCenter
    AutoSize = False
    Caption = 'This information can also be given in the'
  end
  object Label6: TLabel
    Left = 24
    Top = 112
    Width = 217
    Height = 13
    Alignment = taCenter
    AutoSize = False
    Caption = '"<var>" environment variable'
  end
  object Label2: TLabel
    Left = 8
    Top = 8
    Width = 182
    Height = 13
    Caption = 'The name for this server was given as:'
  end
  object Label3: TLabel
    Left = 16
    Top = 24
    Width = 67
    Height = 13
    Caption = '<servername>'
  end
  object ValEdit: TEdit
    Left = 8
    Top = 64
    Width = 249
    Height = 21
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    Text = 'localhost'
  end
  object BitBtn1: TBitBtn
    Left = 100
    Top = 136
    Width = 75
    Height = 25
    Caption = 'OK'
    Default = True
    ModalResult = 1
    TabOrder = 1
    Glyph.Data = {
      
      5AA0555555555555577755555555555555555555555555555555}
    NumGlyphs = 2
  end
  object BitBtn2: TBitBtn
    Left = 184
    Top = 136
    Width = 75
    Height = 25
    Cancel = True
    Caption = 'Cancel'
    ModalResult = 2
    TabOrder = 2
    Glyph.Data = {
      
      3333333777333777333333333999993333333333377777333333}
    NumGlyphs = 2
  end
end
